...
ex4_rand_const <- read.csv("p_rand_ const.csv", dec = ".", head = TRUE, sep = ",")
ggplot(data = ex4_rand_const, aes(x = n, y = time, color = factor(sort))) + geom_line() + ggtitle("Random & Constant data") +
  theme(legend.text = element_text(size = 14), legend.title = element_text(size = 12)) + theme_gray(base_size = 18)

ex4_incr_decr <- read.csv("p_incr_decr.csv", dec = ".", head = TRUE, sep = ",")
ggplot(data = ex4_incr_decr, aes(x = n, y = time, color = factor(sort))) + geom_line() + ggtitle("Increasing & Decreasing data") +
  theme(legend.text = element_text(size = 14), legend.title = element_text(size = 12)) + theme_gray(base_size = 18)

ex4_a_v <- read.csv("p_a_v.csv", dec = ".", head = TRUE, sep = ",")
ggplot(data = ex4_a_v, aes(x = n, y = time, color = factor(sort))) + geom_line() + ggtitle("A-shape & V-shape data") +
  theme(legend.text = element_text(size = 14), legend.title = element_text(size = 12)) + theme_gray(base_size = 18)
